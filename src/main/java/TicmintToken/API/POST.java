    ...
    public static void run() {
        ...
        post("/lockInPreparationForWithdrawal/", (request, response) -> {
            try {
                ...
                if(Settings.tmt.getUserBalance(usersAddress).compareTo(amount) < 0) return getFail("message", "Not enough balance");
        ...
        post("/unlockFromWithdrawal/", (request, response) -> {
            try {
                ...
                if(Settings.tmt.getUserLockedBalance(usersAddress).compareTo(amount) < 0) return getFail("message", "Not enough locked balance");
